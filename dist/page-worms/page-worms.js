...
// page-worms/constants.ts
var DEFAULTS = {
  wormClass: "pw-worm",
  // css class for worm elements
  wormActiveClass: "pw-worm-active",
  // css class for active worm elements
  maxTextContext: 256,
  // max chars for text quote context (prefix + suffix)
  throttleMs: 60,
  // resize/scroll/mutation observers throttle delay
  storageKeyPrefix: "pageworms::"
  // used for local storage (both chrome and local storage adapter)
};
var PW_OWNED_ATTR = "data-pw-owned";
var PW_OWNED_SELECTOR = `[${PW_OWNED_ATTR}]`;
var PW_OWNED_DATASET_KEY = "pwOwned";
var PW_OWNED_DATASET_VALUE = "1";

// page-worms/utils.ts
function uuid() {
  const cryptoObj = typeof globalThis.crypto !== "undefined" ? globalThis.crypto : void 0;
...
var normalizeText = (s) => (s ?? "").toString().normalize("NFC").replace(/\s+/g, " ").trim();
...
// page-worms/anchoring/anchoring-helpers.ts
function getCssPath(el) {
  ...
  while (el && el.nodeType === Node.ELEMENT_NODE && el !== document.body) {
    ...
    const getStableAttrs2 = attrs.filter((a) => /^data-|^aria-|^role$/.test(a.name)).slice(0, 2).map((a) => `[${a.name}="${a.value}"]`).join("");
    if (getStableAttrs2) tag += getStableAttrs2;
    let idx = 1, sib = el;
...
function getAllTextNode(root = document.body ?? document) {
...
function getQuoteRange(quote, prefix = "", suffix = "", cached) {
  function findBestMatch(allText2, quote2, prefix2 = "", suffix2 = "") {
    ...
    const hits = [];
    let idx = -1, from = 0;
    while ((idx = allText2.indexOf(quote2, from)) !== -1) {
      const pre = allText2.slice(Math.max(0, idx - prefix2.length), idx);
      const suf = allText2.slice(
        idx + quote2.length,
        idx + quote2.length + suffix2.length
      );
      let score = 1;
      if (prefix2) score += commonOverlapLen(pre, prefix2);
      if (suffix2) score += commonOverlapLen(suf, suffix2);
      hits.push({ idx, score });
      from = idx + 1;
    }
...
function getClickRelativePos(el, clientX, clientY) {
...
function selectionContext(range, max) {
  function resolveBoundary(range2, side) {
    const isStart = side === "start";
    const container = isStart ? range2.startContainer : range2.endContainer;
    const offset = isStart ? range2.startOffset : range2.endOffset;
  ...
  function grabText(startNode2, startOffset2, maxChars, direction) {
    let collectedText = "";
    let currentNode = startNode2;
    const shouldSliceFirst = startNode2?.nodeType === Node.TEXT_NODE;
    let isFirstIteration = true;
    while (currentNode && maxChars > 0) {
      if (currentNode.nodeType === Node.TEXT_NODE && currentNode.parentElement && !currentNode.parentElement.matches(
        "script,style,noscript,template"
      )) {
        const normalized = normalizeText(currentNode.nodeValue || "");
        if (direction < 0) {
          collectedText = (isFirstIteration && shouldSliceFirst ? normalized.slice(0, startOffset2) : normalized) + collectedText;
        } else {
          collectedText += isFirstIteration && shouldSliceFirst ? normalized.slice(startOffset2) : normalized;
        }
      ...
      if (!sibling) {
        ...
        currentNode = direction < 0 ? currentNode.previousSibling : currentNode.nextSibling;
        if (!currentNode) break;
        while (direction < 0 ? currentNode.lastChild : currentNode.firstChild) {
          currentNode = direction < 0 ? currentNode.lastChild : currentNode.firstChild;
        }
      } else {
        currentNode = sibling;
      }
      if (currentNode && currentNode.nodeType === Node.TEXT_NODE) {
        startOffset2 = direction < 0 ? (currentNode.nodeValue || "").length : 0;
      }
      isFirstIteration = false;
    }
    return direction < 0 ? collectedText.slice(-maxChars) : collectedText.slice(0, maxChars);
  }
...
function getCoarseContainer(el) {
  function getBlockAncestor(el2, maxHops = 4) {
    let cur = el2;
    let hops = 0;
    ...
    return el2.closest("div,section,article,main") || el2;
  }
...
// page-worms/anchoring/anchoring-adapter.ts
...
var DomAnchoringAdapter = class {
  ...
  createPosition({
    ...
    const baseEl = selection ? getRangeContainer(selection) : target instanceof Element ? target : target && "parentElement" in target ? target.parentElement : null;
    const fineEl = baseEl ?? document.body ?? document.documentElement ?? document.createElement("div");
    ...
    const selectionRect = selection && !selection.collapsed ? selection.getBoundingClientRect() : null;
    if (selectionRect && selectionRect.width > 0 && selectionRect.height > 0 && coarseHost) {
  ...
  resolvePosition(position, cache) {
    const exact = position.textQuote?.exact ? normalizeText(position.textQuote.exact) : "";
    const tryQuery = (sel) => {
      if (!sel) return null;
      try {
    ...
    if (exact) {
      ...
      if (range) {
        const hostEl2 = getRangeContainer(range);
        if (hostEl2 instanceof HTMLElement) return hostEl2;
        if (hostEl2 === document.body && position.dom.selectorFine) {
    ...
    if (tag) {
      ...
      hostEl = cands.find(
        (el) => Object.keys(want).every((k) => {
          ...
          if (k.startsWith("data-")) return got.startsWith(want[k]);
          return got === want[k];
        })
      ) || null;
    }
...
function createAnchoringAdapter() {
...
async function readStoredSession() {
  ...
  try {
    ...
    if (stored && typeof stored.access_token === "string" && typeof stored.refresh_token === "string" && Number.isFinite(stored.expires_at)) {
      return stored;
    }
  } catch (err) {
...
async function persistSessionFromSupabase(session) {
  const next = {
    access_token: session.access_token,
    refresh_token: session.refresh_token || "",
    expires_at: session.expires_at || Math.floor(Date.now() / 1e3)
  };
...
var SupabaseStorageAdapter = class {
  async list(url) {
    ...
    const { data, error } = await connection.client.from("worms").select("id, created_at, updated_at, content, status, tags, author_id, position, host_url").eq("host_url", url).order("created_at", { ascending: true });
    if (error || !data) {
      console.error("[SupabaseAdapter] Failed to list worms:", error);
      return [];
    }
    return data.map((row) => normalizeWorm(row)).filter((worm) => shouldDisplayWorm(worm, mode, profile.idNumber, friendIdSet));
  }
  async create(url, payload) {
    ...
    const { data, error } = await client.from("worms").insert(insertPayload).select("id, created_at, updated_at, content, status, tags, author_id, position, host_url").single();
    if (error || !data) {
      throw new Error(error?.message || "Failed to create worm");
    }
    return normalizeWorm(data);
  }
  async update(url, wormId, updates) {
    const { client, session } = await requireSupabaseConnection();
    const profile = await fetchViewerProfile(client, session.user.id);
    if (!profile?.idNumber) {
      throw new Error("Unable to resolve your profile. Please complete onboarding.");
    ...
    const { data, error } = await client.from("worms").update(updatePayload).eq("id", wormId).eq("author_id", profile.idNumber).select("id, created_at, updated_at, content, status, tags, author_id, position, host_url").single();
    if (error || !data) {
      throw new Error(error?.message || "Failed to update worm");
    }
    return normalizeWorm(data);
  }
  async remove(url, wormId) {
    const { client, session } = await requireSupabaseConnection();
    const profile = await fetchViewerProfile(client, session.user.id);
    if (!profile?.idNumber) {
      throw new Error("Unable to resolve your profile. Please complete onboarding.");
    }
    const { error } = await client.from("worms").delete().eq("id", wormId).eq("author_id", profile.idNumber);
    if (error) {
      throw new Error(error.message || "Failed to delete worm");
    }
  }
};
async function safeReadDisplayMode() {
  try {
...
function shouldDisplayWorm(worm, mode, viewerProfileId, friendIds) {
  if (mode === "private") {
...
async function fetchViewerProfile(client, authId) {
  const { data, error } = await client.from("profiles").select("id, auth_id").eq("auth_id", authId).maybeSingle();
  if (error) {
...
async function fetchFriendProfileIds(client, viewerAuthId) {
  const { data, error } = await client.from("friend_edges").select("a, b, state").or(`a.eq.${viewerAuthId},b.eq.${viewerAuthId}`).eq("state", "accepted");
  if (error || !data) {
    if (error) {
  ...
  const friendAuthIds = /* @__PURE__ */ new Set();
  for (const edge of data) {
    const other = edge.a === viewerAuthId ? edge.b : edge.b === viewerAuthId ? edge.a : null;
    if (other) friendAuthIds.add(other);
  }
  if (!friendAuthIds.size) return [];
  const { data: profiles, error: profileError } = await client.from("profiles").select("id, auth_id").in("auth_id", Array.from(friendAuthIds));
  ...
  return profiles.map((row) => normalizeNumber(row.id)).filter((id) => typeof id === "number");
}

// page-worms/storage/storage-adapter.ts
function normalizeTags(tags) {
  return tags && tags.length ? tags : null;
}
function cloneWorm(worm) {
  if (typeof structuredClone === "function") {
    return structuredClone(worm);
  }
  return JSON.parse(JSON.stringify(worm));
}
var ArrayStorageAdapter = class {
  async list(url) {
  ...
  async remove(url, wormId) {
    ...
    if (next.length === worms.length) return;
...
function parseStoredWorms(value) {
  if (Array.isArray(value)) return value;
  return [];
}
var LocalStorageAdapter = class extends ArrayStorageAdapter {
  ...
  async write(url, worms) {
    if (typeof localStorage === "undefined") return;
    localStorage.setItem(this.keyFor(url), JSON.stringify(worms));
  }
};
var ChromeStorageAdapter = class extends ArrayStorageAdapter {
  constructor(prefix = DEFAULTS.storageKeyPrefix) {
    super();
    this.prefix = prefix;
  }
...
function createStorageAdapter(storageOption = "remote") {
...
// page-worms/observer/observer-adapter.ts
var BrowserObserverAdapter = class {
  constructor() {
  ...
  start(options) {
    ...
    this.mutationObserver = new MutationObserver(
      (entries) => {
        if (!this.scheduleRender || !this.isManagedNode) return;
        if (entries.length === 0) return;
        for (const record of entries) {
          if (this.isManagedNode(record.target) || [...record.addedNodes].some((node) => this.isManagedNode(node)) || [...record.removedNodes].some((node) => this.isManagedNode(node))) {
            continue;
          }
          this.scheduleRender();
          break;
...
function createObserverAdapter() {
  return new BrowserObserverAdapter();
}

// page-worms/rendering/dom-layer.ts
function createWormEl() {
...
function makePositioningContext(containerEl) {
...
function createOrUpdateBox(containerEl, hostEl, idGen) {
...
// page-worms/styles.ts
var STYLE_ID = "pw-style-link";
...
// page-worms/rendering/rendering-adapter.ts
var DomRenderingAdapter = class {
  constructor({ anchoringAdapter, observerAdapter, wireWormElement }) {
    // DOM state cached so we can reuse elements between renders.
    this.wormEls = /* @__PURE__ */ new Map();
    this.hostByWorm = /* @__PURE__ */ new Map();
  ...
  drawWorm(worm) {
    ...
    const targetContainer = cannotContain ? createOrUpdateBox(containerEl, host, uuid) : containerEl;
  ...
  resolveRenderContext(worm) {
    ...
    const containerEl = cannotContain ? host.parentElement instanceof HTMLElement ? host.parentElement : document.body ?? host : host;
...
function createRenderingAdapter(deps) {
  return new DomRenderingAdapter(deps);
}

// page-worms/ui/ui-manager/templates.ts
var OWNED_ATTR_FRAGMENT = `${PW_OWNED_ATTR}=""`;
var tooltipMarkup = `
...
// page-worms/ui/ui-manager/ui.ts
var STATUS_LABELS = {
  private: "Private",
  friends: "Friends",
  public: "Public"
...
var WormUI = class {
  ...
  _handleModalSubmit(e) {
    ...
    const status = statusValue === "friends" || statusValue === "public" ? statusValue : "private";
    this._resolveForm({
      content,
      tags,
      status
  ...
  _setModalContent(contentEl, state) {
    ...
    this._windowEl.dataset.wormId = state?.wormId != null ? String(state.wormId) : "";
  ...
  async _confirmDelete(wormId) {
    ...
    const ok = typeof confirm === "function" ? confirm(msg) : typeof window !== "undefined" && typeof window.confirm === "function" ? window.confirm(msg) : true;
    if (ok === false) return;
    this.closeModal();
    try {
...
// page-worms/ui/ui-adapter.ts
var DomUIAdapter = class {
  constructor(deps) {
    this.ui = new WormUI({
...
var PageWorms = class {
  ...
  _isManagedNode(node) {
    ...
    return !!(el && typeof el.closest === "function" && el.closest(PW_OWNED_SELECTOR));
  }
  // #endregion
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  _handleStorageError(action, err) {
    try {
...
async function attachPageWorms(storageOption) {
