...
export class WormUI {
    ...
    _handleModalClick(e) {
        ...
        switch (action) {
            ...
            case "edit": {
                if (wormId != null)
                    void this._beginEdit(wormId);
                break;
            }
            case "delete": {
                if (wormId != null)
                    void this._confirmDelete(wormId);
                break;
            }
            default:
                break;
    ...
    _handleModalSubmit(e) {
        ...
        const status = statusValue === "friends" || statusValue === "public"
            ? statusValue
            : "private";
        this._resolveForm({
            content,
    ...
    _handleModalKeydown(e) {
        if (e.key !== "Escape")
            return;
        if (!this._backdropEl || this._backdropEl.hidden)
            return;
        e.preventDefault();
        const state = this._state;
        this._resolveForm(null);
    ...
    async _openForm({ mode, worm, }) {
        ...
        if (textarea)
            textarea.value = worm?.content || "";
        if (tagsInput)
            tagsInput.value = Array.isArray(worm?.tags) ? worm.tags.join(", ") : "";
        if (statusSelect)
            statusSelect.value = worm?.status || "private";
