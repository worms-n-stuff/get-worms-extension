...
// TextCache helper
function buildTextCacheForRoot(root: Element | Document): DomAnchorCache {
...
class DomAnchoringAdapter implements AnchoringAdapter {
  ...
  createPosition({
    ...
    const selectorCoarse =
      coarseEl && coarseEl !== fineEl ? getCssPath(coarseEl) : selectorFine;
    ...
    const coarseHost =
      coarseEl instanceof HTMLElement ? coarseEl : (coarseEl as HTMLElement | null);
    let hostForRel: HTMLElement = fineEl;
    const selectionRect =
    ...
    let relClientX = Number.isFinite(clickX) ? (clickX as number) : NaN;
    let relClientY = Number.isFinite(clickY) ? (clickY as number) : NaN;
  ...
  resolvePosition(
    ...
    const exact = position.textQuote?.exact
      ...
      : "";
    const tryQuery = (sel?: string | null): HTMLElement | null => {
      if (!sel) return null;
      try {
