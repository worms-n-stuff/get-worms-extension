...
class BrowserObserverAdapter implements ObserverAdapter {
  private scheduleRender: (() => void) | null = null;
  private isManagedNode: ((node: Node | null) => boolean) | null = null;
    ...
    this.mutationObserver = new MutationObserver(
      (entries: MutationRecord[]) => {
        ...
        for (const record of entries) {
          if (
            this.isManagedNode(record.target) ||
  ...
  private ensureHostObserver(): ResizeObserver {
...
export function createObserverAdapter(): ObserverAdapter {
