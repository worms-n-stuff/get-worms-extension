...
export async function ensureWormsToggle(defaultValue = false): Promise<boolean> {
  try {
...
export async function readWormsToggle(): Promise<boolean> {
  try {
    const result = await chrome.storage.sync.get(PW_TOGGLE_KEY);
    return Boolean(result?.[PW_TOGGLE_KEY]);
  } catch {
...
export async function writeWormsToggle(enabled: boolean): Promise<void> {
